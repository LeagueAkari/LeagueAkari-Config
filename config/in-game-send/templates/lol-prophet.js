...
function extractGame(puuid, gameSummary) {
  ...
  const getStats = (participant) => (source === 'lcu' ? participant.stats : participant)

  const totalKills = sumBy(mates, (p) => getStats(p).kills || 0)
  const totalDmg = sumBy(mates, (p) => getStats(p).totalDamageDealtToChampions || 0)
  ...
  // ---------- 排名 ----------
  const rank = (arr) => arr.findIndex((p) => participantIdentifier(p) === meIdentifier)
  ...
  const damageShare = (meStats.totalDamageDealtToChampions || 0) / noZero(totalDmg)
  ...
  const gameCreation = source === 'lcu' ? game.gameCreation : game.json.gameCreation

  const rawDuration = source === 'lcu' ? game.gameDuration : game.json.gameDuration
  const safeDuration = Math.max(rawDuration || 0, 1)
  const durationMinutes = safeDuration / 60

  const totalCs = (meStats.totalMinionsKilled || 0) + (meStats.neutralMinionsKilled || 0)
...
function buildPlayerLine({ env, puuid, score }) {
  const inChampSelect =
    env.queryStage?.phase === 'champ-select' || env.queryStage?.phase === 'lobby'
...
function getMessages(env) {
  const isPveQueue =
    typeof env.utils?.isPveQueue === 'function' ? env.utils.isPveQueue : () => false
  ...
  const players = targets.map((puuid) => {
    ...
    const valid = mh.data.filter((g) => g && g.data).filter((g) => !isPveQueue(g.data.queueId))
  ...
  return players.map((p) => buildPlayerLine({ env, puuid: p.puuid, score: p.score }))
}
