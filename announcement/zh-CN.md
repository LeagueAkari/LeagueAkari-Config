...
## 公告板 (发布于 2025-12-30 22:00)

圣诞节快乐。

目前最新版本是 v1.4.0，更新于 2025-12-25。

## <span class="flow-gradient-text-9e2b3c0b">(新)</span> 已知问题

1. 启动页面的快速客户端启动方式，部分用户无法使用。
2. 部分页面未适配亮色模式。
3. 使用 WMI 方式连接客户端，CPU 占用高。
4. 开黑判定有时候有问题。
5. 战绩页面翻页时重置了每页数量。
6. 对局玩家卡片的一些标签没有被正确展示。
7. 发送模板出现问题。
8. 部分页面遗漏适配亮色模式。
9. OP.GG 窗口部分用户无法正常启用。
10. 自动选择/禁用类功能在某些情况下失效，以及在设置延后后时间计算错误。乱斗模式的英雄选择自动交换功能被错误地强制开启。
11. 对于某些匿名的玩家，它们的分配位置（如中路、上路、...）信息没有被补全。
...
1. 群聊会定期清理潜水人员。
2. 请确保你掌握基础的计算机常识。如，可以正确使用解压缩软件，知晓“快捷方式”和“主程序”的区别，知道如何发送日志文件，以及会高效且正确地描述问题。
