...
## <span class="flow-gradient-text-9e2b3c0b">(新)</span> 公告板 (发布于 2025-11-20 22:43)

相信细心的小伙伴已经注意到了，在“对局”功能中，部分玩家不可见。这是由于该玩家开启了“主播模式”，目前版本 League Akari 暂时无法获取开启了主播模式玩家的信息。

另外，由于客户端更新，LCU 观战暂无法使用，将在下个版本修复。

---

在使用 TCLS 启动腾讯服客户端时，其会自动将 `startup_runner.exe` 加入到自启项，导致开机启动。此问题与 League Akari 无关。

解决方案：可以使用 **任务管理器** 中的 **启动** 选项卡，手动将 `startup_runner.exe (或 “英雄联盟登录程序”)` 从自启项禁用。

## 当前已知问题 & 版本发布时间线

目前最新版本是 v1.3.7。下个主要版本将**最晚**在今年 ~~12 月之前~~年底之前发布（没错又鸽了）。

- [BUG] 一些地方错误地展示英文 (如 「自动选择」 在聊天框中提示 `auto-select-main.delayed-ban` 等)
- [BUG] 在**空载**时，应用仍大量占用 CPU。此现象暂无法在开发侧复现，若有可稳定复现的案例，请务必进入群聊反馈。
- [BUG] 目前玩家标记导出和导入逻辑存在问题。已知导入逻辑、导出逻辑在数据量过大（或其他情况）的时候会失败。
- [需迁移] 目前 wmic 已被微软弃用，导致依赖 wmic 查询命令行的途径失效，影响与客户端连接、安装位置检测等相关功能。下个版本将使用现代 WMI API。
